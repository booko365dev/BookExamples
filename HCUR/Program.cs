...
//---------------------------------------------------------------------------------------
// ------**** ATTENTION **** This is a DOTNET 10 Console Application ****----------
//---------------------------------------------------------------------------------------
#nullable disable
...
//gavdcodebegin 001
static void WordOpenXml_CreateDocument()
{
	...
	docRun.AppendChild(new Text("Text in the Word document"));
}
//gavdcodeend 001

//gavdcodebegin 002
static void WordOpenXml_CreateDocumentFromStream()
{
	FileStream myStream = File.Create(@"C:\Temporary\WordDoc02.docx");

	using WordprocessingDocument myWordDoc = WordprocessingDocument.Create(myStream,
												  WordprocessingDocumentType.Document);
	...
	Paragraph docParagraph = docBody.AppendChild(new Paragraph());
	Run docRun = docParagraph.AppendChild(new Run());
	docRun.AppendChild(new Text("Text from stream Word document"));
}
//gavdcodeend 002

//gavdcodebegin 003
static void WordOpenXml_OpenDocument()
{
	using WordprocessingDocument myWordDoc =
...
//gavdcodebegin 004
static void WordOpenXml_OpenAndAddTextToDocument()
{
	using WordprocessingDocument myWordDoc =
...
//gavdcodebegin 005
static void WordOpenXml_FindStandardPropertiesDocument()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", false);

	ExtendedFilePropertiesPart docPart = myWordDoc.ExtendedFilePropertiesPart;
	DocumentFormat.OpenXml.ExtendedProperties.Properties myProps = docPart.Properties;
...
//gavdcodebegin 026
static void WordOpenXml_FindCustomPropertiesDocument()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", false);

	CustomFilePropertiesPart docPart = myWordDoc.CustomFilePropertiesPart;
	DocumentFormat.OpenXml.CustomProperties.Properties myProps = docPart.Properties;
...
//gavdcodebegin 006
static void WordOpenXml_AddOneCustomPropertyDocument()
{
	...
	if (customProps != null)
	{
		...
		int pid = 2;
		foreach (CustomDocumentProperty item in
									customProps.Cast<CustomDocumentProperty>())
		{
			item.PropertyId = pid++;
		}
		customProps.Save();
...
//gavdcodebegin 007
static void WordOpenXml_JoinTwoDocuments()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
...
//gavdcodebegin 008
static void WordOpenXml_FontForRun()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
...
//gavdcodebegin 009
static void WordOpenXml_FindStyles()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", false);
...
//gavdcodebegin 010
static void WordOpenXml_CreateAndApplyStyleParagraph()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	ParagraphProperties paragraphProps = new()
	{
...
//gavdcodebegin 011
static void WordOpenXml_CreateHeader()
{
	using WordprocessingDocument myWordDoc =
		WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	MainDocumentPart docMainPart = myWordDoc.MainDocumentPart;
...
static void GenerateHeaderPartContentFirst(HeaderPart firstPart)
{
	Header hFirst = new()
	{
	...
	Paragraph pFirst = new()
	{
		RsidParagraphAddition = "00164C17",
		RsidRunAdditionDefault = "00164C17"
	};
	ParagraphProperties pPropFirst = new();
	ParagraphStyleId pStyleIdFirst = new() { Val = "Header" };
...
//gavdcodebegin 012
static void WordOpenXml_CreateFooter()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
...
static void GenerateFooterPartContent(FooterPart defaultPart)
{
	Footer fDefault = new()
	{
		MCAttributes = new MarkupCompatibilityAttributes()
	...
	Paragraph pDefault = new()
	{
		RsidParagraphAddition = "00164C17",
		RsidRunAdditionDefault = "00164C17"
	};

	ParagraphProperties pPropDefault = new();
	ParagraphStyleId pStyleIdDefault = new() { Val = "Footer" };
	...
	{
		Text = "Footer in all pages [default]"
	};
...
//gavdcodebegin 013
static void WordOpenXml_RemoveHeadersAndFooters()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	if (docMainPart.HeaderParts.Any() || docMainPart.FooterParts.Any())
	{
		docMainPart.DeleteParts(docMainPart.HeaderParts);
		docMainPart.DeleteParts(docMainPart.FooterParts);
...
//gavdcodebegin 014
static void WordOpenXml_AddComment()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	if (myWordDoc.MainDocumentPart.GetPartsOfType<WordprocessingCommentsPart>().Any())
	{
		myComments = myWordDoc.MainDocumentPart.WordprocessingCommentsPart.Comments;
		if (myComments.HasChildren)
		{
			id = myComments.Descendants<Comment>().Select(e => e.Id.Value).Max();
		}
	}
	else
	{
	...
	Paragraph myParagraph = new(
								new Run(
									new Text("This is a new comment")));
...
//gavdcodebegin 015
static void WordOpenXml_FindCommentsinDocument()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", false);
...
//gavdcodebegin 016
static void WordOpenXml_DeleteComments()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	IEnumerable<string> commentIds = commentsToDelete.Select(del => del.Id.Value);

	foreach (Comment myComment in commentsToDelete)
	{
		myComment.Remove();
	}
...
//gavdcodebegin 017
static void WordOpenXml_AddTableUsingData()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);

	Document myDocument = myWordDoc.MainDocumentPart.Document;

	Table newTable = new();

	TableProperties tableProps = new(  // Table properties
		new TableBorders(
			new TopBorder
			{
...
//gavdcodebegin 018
static void WordOpenXml_AddTableUsingDirectData()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	TableProperties tableProp = new(  // Table properties
		new TableBorders(
	...
	TableRow tblRow = new();  //New row

	TableCell tblCell01 = new();  // New cell
	...
	TableCell tblCell02 = new(tblCell01.OuterXml);
...
//gavdcodebegin 019
static void WordOpenXml_ModifyTextInCell()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	myText.Text = "New text abc123";
}
//gavdcodeend 019

//gavdcodebegin 020
static void WordOpenXml_DeleteRow()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
...
//gavdcodebegin 021
static void WordOpenXml_AddPictureInDocument()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
...
(@"C:\Temporary\MyImage.gif", FileMode.Open))
	{
					 ...
					 new DocumentFormat.OpenXml.Drawing.GraphicData(
						 new DocumentFormat.OpenXml.Drawing.Pictures.Picture(
							 ...
							 new DocumentFormat.OpenXml.Drawing.Pictures.
								 ...
								 {
									 Embed = docMainPart.GetIdOfPart(docImagePart),
									 CompressionState = DocumentFormat.OpenXml.Drawing.
...
//gavdcodebegin 022
static void WordOpenXml_DeleteAllPicturesFromDocument()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
	...
	List<ImagePart> listImageParts = new();
	List<Drawing> listDrwParts = new();
	List<Drawing> listDrwdDeleteParts = [];
	...
	foreach (ImagePart oneImage in docImageParts)
	{
		...
		{
			if (oneDrawing != null && !listDrwdDeleteParts.Contains(oneDrawing))
...
//gavdcodebegin 023
static void WordOpenXml_AcceptAllChages()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);
...
//gavdcodebegin 024
static void WordOpenXml_RemoveHiddenText()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);

	NameTable myNameTable = new();
	XmlNamespaceManager nameManager = new(myNameTable);
	nameManager.AddNamespace("w",
		"http://schemas.openxmlformats.org/wordprocessingml/2006/main");

	XmlDocument myXmlDoc = new(myNameTable);

	Stream myWordDocStream = myWordDoc.MainDocumentPart.GetStream();
	myXmlDoc.Load(myWordDocStream);
	myWordDocStream.Close();

	XmlNodeList allHiddenNodes = myXmlDoc.SelectNodes("//w:vanish", nameManager);
...
//gavdcodebegin 025
static void WordOpenXml_Validator()
{
	using WordprocessingDocument myWordDoc =
				WordprocessingDocument.Open(@"C:\Temporary\WordDoc01.docx", true);

	OpenXmlValidator myValidator = new();
	foreach (ValidationErrorInfo oneError in myValidator.Validate(myWordDoc))
	{
...
//WordOpenXml_CreateDocument();
//WordOpenXml_CreateDocumentFromStream();
//WordOpenXml_OpenDocument();
//WordOpenXml_OpenAndAddTextToDocument();
//WordOpenXml_FindStandardPropertiesDocument();
//WordOpenXml_FindCustomPropertiesDocument();
//WordOpenXml_AddOneCustomPropertyDocument();
//WordOpenXml_JoinTwoDocuments();
//WordOpenXml_FontForRun();
//WordOpenXml_FindStyles();
//WordOpenXml_CreateAndApplyStyleParagraph();
//WordOpenXml_CreateHeader();
//WordOpenXml_CreateFooter();
//WordOpenXml_RemoveHeadersAndFooters();
//WordOpenXml_AddComment();
//WordOpenXml_FindCommentsinDocument();
//WordOpenXml_DeleteComments();
//WordOpenXml_AddTableUsingData();
//WordOpenXml_AddTableUsingDirectData();
//WordOpenXml_ModifyTextInCell();
//WordOpenXml_DeleteRow();
//WordOpenXml_AddPictureInDocument();
//WordOpenXml_DeleteAllPicturesFromDocument();
//WordOpenXml_AcceptAllChages();
//WordOpenXml_RemoveHiddenText();
//WordOpenXml_Validator();
